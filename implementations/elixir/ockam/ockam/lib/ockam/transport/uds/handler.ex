defmodule Ockam.Transport.UDS.Handler do
  @moduledoc false
  ...
  def handle_info(other, state) do
    Logger.warn("UDS HANDLER Received unknown message #{inspect(other)} #{inspect(state)}")
    {:noreply, state}
  end
  ...
  def handle_message(
    ...
    case Ockam.Wire.encode(forwarded_message) do
      {:ok, encoded} ->
        ...
        case byte_size(encoded) <= UDS.packed_size_limit() do
          true ->
      ...
      a ->
        Logger.error("UDS transport send error #{inspect(a)}")
        raise a
    end
